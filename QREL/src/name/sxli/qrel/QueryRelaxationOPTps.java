 ...
 */
public class QueryRelaxationOPTps implements IQueryRelaxation {

	@Override
	public int[] relaxQuery(ExampleGraphAgent graphAgent, OracleAgent oracleAgent, int D, int... queryEntities) throws Exception {
		...
		int[] maxDists=new int[queryEntities.length]; //max distance that no more than D to any v∈VQ
...
//			loopCount++;
			...
			if(Math.floor(v.priority)<=1 || Math.floor(v.priority)<=Qmax.size())
				...
				if(!checked[v.vid]){
					List<Integer> Qv=SubQuery.OptWithCert(graphAgent, oracleAgent, D, queryEntities, v.vid, Qmax);
				...
				if(length<pathDist){
					...
					for(int[] edge:neighborEdges){
						...
						if(!explored[v.svid][v1] && length1==length+1){ //only explore a shortest path
							double prv1=1;
							int maxDist=0; //max distance that no more than D to any v∈VQ
